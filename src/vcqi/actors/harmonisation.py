...
HARMONISATION_ITEMS: tuple[HarmonisationItem, ...] = (
    ...
    HarmonisationItem(
        ...
        demonstrated=(
            ...
            "UN/CEFACT's Digital Conformity Credential and really validated against the "
            "published UNTP schema, which is vendored here so the answer cannot change "
            "without a visible diff. The panel above this list reports what the mapping "
            "could not carry and what the validator says about the result."
        ),
        exists=(
            ...
            "pass or fail, and a measured result.\n\n"
            "Projecting a certificate into it sharpened that reading. The envelope "
            "reaches further into metrology than its reputation suggests — "
            "attestationType already enumerates calibration, assessmentLevel already "
            "separates GlobalMRA from Accredited, and the accreditation behind a body "
            "maps onto authorisation exactly as UNTP describes it. What does not arrive "
            "is the measurement. Conformance is a required boolean where a calibration "
            "reports a value; conformityTopic is required and drawn from fifteen "
            "sustainability codes, none of which a resistance calibration belongs to; "
            "and the two classes that hold a measured value are closed to extension, so "
            "uncertainty has nowhere to go. The nearest member is a plain accuracy "
            "fraction, which is a bound rather than a coverage interval.\n\n"
            "So the honest reading is that the quality infrastructure has one mature "
            "format without standing and one standing format whose envelope reaches "
            "calibration while its payload stops short of the measurement, and nobody "
            "has joined them.\n\n"

<!-- 07-traceability.md -- chapter 6 in the rail; the NN- prefix is the position in the
     ...
     The blocks from combining.title down were a chapter of their own until they were
     folded in here; the comment above them says what stayed in the code. -->
...
<!-- The blocks from combining.title downwards were their own chapter until the
     dependencies material was folded in here. They render after the four representation
     tabs and before the recompute panel, because combining two results is the payoff of
     the identifiers the unclib block promises: this is where an influence that travels
     with its identifier earns its keep. The verdict line under the two columns is
     computed from the numbers and stays in the code; only the one fixed sentence it can
     show instead -- "optimistic" below -- is here. The operation chips and the table
     headers are labels and stay in the code too. -->

<!-- block: combining.title -->

Combining two certified results

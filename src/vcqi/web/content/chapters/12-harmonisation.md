...
The newest item on the list was found the same way, and it is one level down from the measurand problem above. A recognition credential says what the entity it names may issue, and the specification gives it one member for that: `outputValidation`, one or more data schemas. A schema can say that a result is a number inside a range. It cannot say which row of a published table applies, nor that the row depends on whether the object being calibrated is a measuring instrument or a material measure. So this demonstration carries a second member beside it, invented here, and the two disagree by construction: the schema admits claims the register then refuses, which is what chapter 5 is for. Every arrangement needing a scope richer than a schema will invent its own member unless one is agreed, and naming it is cheap now and expensive once several bodies have each named their own.
